...
//array for categories (loaded from systems/*.json when available)
const categories = [];
/**
...
// Try to load systems from a JSON file named after the provided system (npm start {NAME} {SYS})
const systemsFile = "./bin/systems/" + (process.argv[3]) + ".json";
if (fs.existsSync(systemsFile)) {
  try {
    ...
    console.error("Error parsing: " + process.argv[3] + " err:", err);
    process.exit(1);
  }
} 
//global calculation
...
if (process.argv.length < 4) {
  console.error("Expected: npm start {NAME} {SYS}");
  process.exit(1);
} else {
  let now = new Date();
